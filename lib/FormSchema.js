...
export const projectValidationSchema = yep.object({
    ...
    phone:  yep.string().required("Phone is required").matches(/^[0-9]+$/, "Only numbers are allowed").min(10, 'Minimum 10 digits are allowed')
    .max(12, 'Maximum 12 digits are allowed'),
    companyName: yep.string().required("Company Name is required").matches(/^[a-zA-Z !@#\$%\^\&*\)\(+=._-]+$/, "Only alphabets are allowed"),
  ...
  export const jobsValidationSchema = yep.object({
    ...
    phone: yep.string().required("Phone is required").matches(/^[0-9]+$/, "Only numbers are allowed").min(10, 'Minimum 10 digits are allowed')
    .max(12, 'Maximum 12 digits are allowed'),
    portfolioLink: yep.string().required("Portfolio link Name is required"),
  ...
  export const serviceValidationSchema = yep.object({
    ...
    .max(12, 'Maximum 12 digits are allowed'),
    companyName: yep.string().required("Company Name is required").matches(/^[a-zA-Z !@#\$%\^\&*\)\(+=._-]+$/, "Only alphabets are allowed"),
    serviceRequired: yep.string().required("Service Required is required").matches(/^[a-zA-Z !@#\$%\^\&*\)\(+=._-]+$/, "Only alphabets are allowed"),
    projectExplanation: yep
      .string()
      .required("Project Explanation is required"),
  });
